...
=======
    ...
    {
>>>>>>> 9abd07bb0535d67ad807800aba854f11aaf61395
...
void setup()
{
    ...
    <<<<<<< HEAD if (settingsStrings[i2cdev] != "")
    {
...
#ifdef ARCH_PORTDUINO
            <<<<<<< HEAD if (settingsMap[use_sx1262])
            {
                if (!rIf)
                {
...
LockingArduinoHal *RadioLibHAL = new LockingArduinoHal(SPI, spiSettings);
            ...
            rangefinder = Rangefind::createRangefinder();
        }
#endif
...
>>>>>>> 9abd07bb0535d67ad807800aba854f11aaf61395
            ...
            if (false)
            {
                // Rangefinding test please ignore
                GeoCoord *current = new GeoCoord(gpsStatus->getLatitude(), gpsStatus->getLatitude(), gpsStatus->getAltitude());
                rangefinder->getDBH();
                float lat = (current->pointAtDistance(rangefinder->getBearing(), rangefinder->getDistance()))->getLatitude();
                float lon = (current->pointAtDistance(rangefinder->getBearing(), rangefinder->getDistance()))->getLongitude();
                float alt = gpsStatus->getAltitude() + rangefinder->getAltitude();
            }
        }
#endif
